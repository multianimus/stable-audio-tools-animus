...
# Updated generate_forever function to use the same parameters
    ...
    def loop_forever():
        while True:
            generate_cond_with_filename(
                filename,  # Pass the filename to ensure it uses the same filename for each generation
                prompt,
                negative_prompt,
                seconds_start,
                seconds_total,
...
# Updated UI with Filename and Generate Forever Button
def create_sampling_ui_with_filename_old(model_config, inpainting=False):
    with gr.Blocks() as ui:
        with gr.Row():
            with gr.Column(scale=6):
                prompt = gr.Textbox(show_label=False, placeholder="Prompt")
                negative_prompt = gr.Textbox(show_label=False, placeholder="Negative prompt")
                filename_textbox = gr.Textbox(label="Filename", placeholder="Enter filename for output")  # New textbox for filename
            generate_button = gr.Button("Generate", variant='primary', scale=1)
            generate_forever_button = gr.Button("Generate Forever", variant='secondary', scale=1)  # New forever button
            ...
            with gr.Column():
                ...
                if inpainting:
                    with gr.Accordion("Inpainting", open=False):
                        ...
                        init_audio_checkbox = gr.Checkbox(label="Do inpainting")
                        init_audio_input = gr.Audio(label="Init audio")
                        init_noise_level_slider = gr.Slider(minimum=0.1, maximum=100.0, step=0.1, value=80, label="Init audio noise level", visible=False)
        ...
        # Button handlers for Generate and Generate Forever
        generate_button.click(fn=generate_cond_with_filename, 
            inputs=[filename_textbox, *inputs],
            outputs=[audio_output, audio_spectrogram_output]
        )
        generate_forever_button.click(fn=generate_forever, inputs=inputs, outputs=[])  # No outputs for "Generate Forever"

    return ui

# Updated UI Function
def create_sampling_ui_with_filename(model_config, inpainting=False):
    with gr.Blocks() as ui:
        with gr.Row():
            with gr.Column(scale=6):
                # Textboxes for prompts
                prompt = gr.Textbox(show_label=False, placeholder="Prompt", lines=2)
                negative_prompt = gr.Textbox(show_label=False, placeholder="Negative Prompt (Optional)", lines=2)
        ...
        # Generate Forever button correctly uses the filename and other inputs
